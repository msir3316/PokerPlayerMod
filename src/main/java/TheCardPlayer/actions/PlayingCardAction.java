...
import TheCardPlayer.vfx.PlayingCardEffect;

import java.util.ArrayList;
import java.util.LinkedList;

import static TheCardPlayer.vfx.PlayingCardEffect.EFFECT_DUR;

public class PlayingCardAction extends AbstractGameAction {
	private static final UIStrings uiStrings = CardCrawlGame.languagePack.getUIString("CardPlayerMod:PokerHands");
	...
	public static LinkedList<ImmutablePair<PlayingCard.Suit, AbstractMonster>> pendingEffects = new LinkedList<>();
	...
	public PlayingCardAction(PlayingCard card) {
	...
	public void update() {
		...
		if (timer <= 0.0f && index < pow.length) {
			if (pow[index] > 0) {
				AbstractDungeon.effectList.add(
						new PlayingCardEffect(PlayingCard.Suit.values()[index])
				);
				timer = DUR_DELTA;
				this.duration = DUR;
				pow[index]--;
		...
		if (pendingEffects.size() > 0) {
			Pair<PlayingCard.Suit, AbstractMonster> smPair = pendingEffects.pop();
