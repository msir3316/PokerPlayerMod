	...
	public static class SendPostPatch {
		...
		public static void Postfix(SingleCardViewPopup __instance, SpriteBatch sb) {
			...
			if (card instanceof PlayingCard) {
				PlayingCard tnc = (PlayingCard) card;

				for (int i = 0; i < tnc.num; i++) {
					float scale = Settings.scale * 2;
					float dx = PlayingCard.OFFSETS_X[tnc.num - 1][i];
					float dy = PlayingCard.OFFSETS_Y[tnc.num - 1][i];
					sb.draw(
							tnc.suit.getImage(),
							(float) Settings.WIDTH / 2.0F - PlayingCard.SUIT_WIDTH,
							(float) Settings.HEIGHT / 2.0F - PlayingCard.SUIT_HEIGHT + 72.0F * Settings.scale,
							PlayingCard.SUIT_WIDTH - dx * 2,
							PlayingCard.SUIT_HEIGHT - 72.0f - dy * 2,
							PlayingCard.SUIT_WIDTH * 2,
							PlayingCard.SUIT_HEIGHT * 2,
							scale,
							scale,
							0.0F,
							0,
