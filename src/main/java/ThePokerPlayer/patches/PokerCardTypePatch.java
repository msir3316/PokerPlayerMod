...
public class PokerCardTypePatch {
	...
	public static class CardFramePatch {
		...
		public static void Insert(SingleCardViewPopup __instance, SpriteBatch sb, AbstractCard card, @ByRef TextureAtlas.AtlasRegion[] tmpImg, @ByRef float[] tOffset, @ByRef float[] tWidth) {
	...
	static TextureAtlas.AtlasRegion region = null;
	...
	public static class FuckRenderFixSwitchesPatch {
		...
		public static SpireReturn<?> Prefix(AbstractCard __instance, SpriteBatch sb, float xPos, float yPos) {
			if (__instance.type == CardTypeEnum.POKER) {
				...
				renderHelper(__instance, sb, region, xPos, yPos);
	...
	private static void renderHelper(AbstractCard card, SpriteBatch sb, TextureAtlas.AtlasRegion region, float xPos, float yPos) {
		try {
