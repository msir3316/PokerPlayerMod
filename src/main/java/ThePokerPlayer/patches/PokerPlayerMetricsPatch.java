...
public class PokerPlayerMetricsPatch {
	...
	public static class SendPostPatch {
		...
		public static void Prefix(Metrics __instance, @ByRef String[] url, String fileName) {
			...
			if (AbstractDungeon.player.chosenClass == ThePokerPlayerEnum.THE_POKER_PLAYER) {
				url[0] = "http://35.185.169.207:12507/upload";
			}
		}
	}
	...
	public static class RunPatch {
		...
		public static void Postfix(Metrics __instance) {
			if (__instance.type == Metrics.MetricRequestType.UPLOAD_METRICS && AbstractDungeon.player.chosenClass == ThePokerPlayerEnum.THE_POKER_PLAYER) {
				try {
