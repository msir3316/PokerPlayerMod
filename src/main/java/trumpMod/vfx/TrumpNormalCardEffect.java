...
public class TrumpNormalCardEffect extends AbstractGameEffect {
	...
	private TrumpNormalCard.Suit suit;
	...
	public TrumpNormalCardEffect(TrumpNormalCard.Suit suit) {
	...
	public void update() {
		if (!this.init) {
			...
			if (suit == TrumpNormalCard.Suit.Heart || suit == TrumpNormalCard.Suit.Spade) {
				addDests(p.hb);
			} else {
				int num = 0;
				for (AbstractMonster m : AbstractDungeon.getCurrRoom().monsters.monsters) {
					if (!m.halfDead && !m.isDying && !m.isEscaping) {
						if (suit == TrumpNormalCard.Suit.Clover) {
							addDests(m.hb);
						} else {
				...
				if (suit == TrumpNormalCard.Suit.Diamond) {
					if (selected == null) {
						isDone = true;
						return;
					}
		...
		if (isDone) {
			TrumpNormalCardAction.pendingEffects.add(new ImmutablePair<>(suit, selected));
		}
	}
	...
	public void render(SpriteBatch sb) {
		...
		for (int i = 0; i < curPos.size(); i++) {
			this.renderImg(sb, TrumpNormalCard.SUIT_TO_IMG[suit.value], rotation, i);
		}
	}
	...
	private void renderImg(SpriteBatch sb, Texture img, float rotation, int loc) {
		...
		sb.draw(img, v.x, v.y,
				TrumpNormalCard.SUIT_WIDTH / 2.0f,
				TrumpNormalCard.SUIT_HEIGHT / 2.0f,
